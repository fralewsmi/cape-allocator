...
Pure functions with no I/O, operating on plain Python floats. They implement
the optimal risky-asset weight for a CRRA investor under Merton (1971).
...
Merton Rule:
    ...
    where:
        f*  = optimal fraction of wealth in the risky asset
        μ   = expected excess return over the risk-free rate
        γ   = coefficient of relative risk aversion (CRRA)
        σ   = expected annualised volatility
...
Application to Excess Earnings Yield:
    ...
    σ = 0.18 (18%) is the constant used by Haghani & White and the
    AllocateSmartly implementation (footnote 2 of their strategy page).
...
def compute_excess_earnings_yield(
    ...
    μ = EY − TIPS_yield,  where EY = 1/CAPE.

    TIPS rather than nominal Treasuries is the right comparator: earnings
    yield is a real return estimate, so the risk-free rate should also be
    real. Using nominal Treasuries would mix units (the "Fed Model" error).
    ...
    -------
    float
        Excess earnings yield (decimal). Negative means TIPS offer a higher
        expected real return than equities.
    """
...
def compute_merton_share(
    ...
    The fraction of wealth a CRRA investor with risk aversion γ should hold
    in equities, given expected excess return μ and volatility σ.
...
def apply_allocation_bounds(
    ...
    -------
    float
        Equity allocation clamped to [min_equity, max_equity].
    """
...
def compute_cer(
    ...
    The risk-free return the investor would accept in place of the risky
    portfolio. Ma et al. (2026) use this in Table 8 to compare allocation
    value across CAPE variants.
